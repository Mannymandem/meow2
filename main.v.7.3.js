 ...
 * 
 * CHANGES:
 *   - parseErrorDescription(...) uses case-insensitive regex 
 *   - If user is above max or below min, we show only "Max Amount: ..." or "Min Amount: ...".
 *   - Otherwise, if we can't decide, we fall back to the aggregator's full message.
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(jsStart>-1){
      ...
      try{
        ...
        if(parsed.description){
          // Example aggregator: "Amount does not fall within the range. min: 10.2, Max: 50.8"
          let desc=parsed.description;

          // If it includes "Amount does not fall within the range."
          // we try to parse min & max (case-insensitive) then compare userAmt
          if(/amount does not fall within the range/i.test(desc)){
            // Case-insensitive regex for min: or max:
          ...
          // If we can't figure out min or max specifically, return aggregator's entire message
          return desc;
        }
      }catch(e){}
    }
  ...
  function updateAmounts(){
    ...
    fetch(url)
      ...
      .then(d=>{
        if(d.error){
          // Call parseErrorDescription with fromAmt
