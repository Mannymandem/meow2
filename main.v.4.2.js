document.addEventListener('DOMContentLoaded', () => {
  ...
  // Instead of having two separate buttons, we now have one button with a lottie animation
  // We'll default isFixed = true => "locked" symbol means fixed rate
  let isFixed = true;

  // HTML elements from the trading window
  ...
  // ======= Single button for toggling fixed vs float =======
  const rateToggleButton = document.getElementById('rate-toggle-button');
  // We'll also update the small paragraph that says "Rate = fixed"
  const rateStatusEl = document.querySelector('.paragraph-2'); 
  // e.g. <p class="paragraph-2">Rate = fixed</p> in your HTML
  ...
  // Snake step references (for status text/spinner in the game overlay)
  ...
  // Network color map (shortened sample)
  const networkColors = {
    "BTC": "#F7931A",
    "ETH": "#3C3C3D",
    "BSC": "#F0B90B",
  ...
  // ----------------------------------------------------------------
  // Single Button => toggle isFixed from true<->false on each click
  // ----------------------------------------------------------------
  if (rateToggleButton) {
    rateToggleButton.addEventListener('click', () => {
      ...
      // If you want to control the lottie animation for lock/unlock, do so here 
      // (e.g. setDirection(1) or setDirection(-1), or play segments).
      // For example:
      // const lottieInstance = window.Webflow.require('lottie').lottie;
      // lottieInstance.play('some-lottie-id'); -- depends on your Webflow setup

      // Re-fetch the updated amounts with new isFixed
      updateAmounts();
    });
  }

  // Optionally: on load, ensure the paragraph says "Rate = fixed" if isFixed = true
  if (rateStatusEl) {
    rateStatusEl.textContent = isFixed ? "Rate = fixed" : "Rate = float";
  }

  // ------------------------------------------------
  // 2) 10-min Countdown Functions
  // ------------------------------------------------
  ...
  function renderCryptoButton(buttonEl, symbol, image, network) {
    ...
    // --- CHANGED: now always pull local icon instead of using aggregator or SimpleSwap fallback ---
    const localIconPath = `/images/crypto-icons/${symbol.toLowerCase()}.svg`;
    const imgSrc = localIconPath;
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect) {
    ...
    cryptos.forEach(coin => {
      ...
      // --- CHANGED: always use local path; no more SimpleSwap fallback ---
      const localIconPath = `/images/crypto-icons/${coin.symbol.toLowerCase()}.svg`;
      const imgSrc = localIconPath;
  ...
  // ------------------------------------------------
  // 8) Update UI after direction change
  // ------------------------------------------------
  ...
  // ------------------------------------------------
  // 10) Update "You Get" (on typed input)
  // ------------------------------------------------
  function updateAmounts() {
    ...
    // We add "is_fixed" param => isFixed can be true or false
  ...
  // ------------------------------------------------
  // 14) Fetch aggregator cryptos & fallback logos
  // ------------------------------------------------
