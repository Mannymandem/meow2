 ...
 *   - The rate toggle updates an optional element #text-container-rate if present.
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(rateToggleButton){
    rateToggleButton.addEventListener('click', () => {
      ...
      if(textContainerRateEl){
        textContainerRateEl.textContent = isFixed ? "Fixed Exchange Rate" : "Floating Exchange Rate";
      }
      updateAmounts();
    });
  }
  ...
  function renderCryptoButton(btn, symbol, image, network){
    ...
    // Use ImageKit for fallback image
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click', () => {
      ...
      .then(d => {
        ...
        // Fill "You Send" using ImageKit fallback
        ...
        // Fill "You Receive" using ImageKit fallback and update rate note
        ...
        // QR Code generation with URI scheme, 5px quiet zone, error correction level H
