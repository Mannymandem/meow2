 ...
 *   - **Dropdown search now filters only by the crypto’s name, ignoring ticker and network.**
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  const aggregatorPollManager = {
    ...
    _fetchAndNotify(aggTxId) {
      ...
      fetch(`${BACKEND_URL}/api/tracking/${aggTxId}`)
        ...
        .then(d => {
          // Notify watchers
          if(!this.pollers[aggTxId]) return; // might have been cleared
  ...
  function setupSearch(sInput, dropdown){
    ...
    sInput.addEventListener('input', () => {
      ...
      items.forEach(item => {
        const searchVal = item.getAttribute('data-search') || ''; // only coin.name
  ...
  /****************************************************
   * 10) buildDropdownItems => store only coin.name in data-search
   ****************************************************/
  function buildDropdownItems(dropdown, cryptos, onSelect){
    ...
    cryptos.forEach(coin => {
      ...
      // Only store coin.name in data-search
      const nameOnly = (coin.name || '').toLowerCase();
      itemEl.setAttribute('data-search', nameOnly);
      ...
      // We'll still display the network pill for clarity, but it's not searched
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click', () => {
      ...
      .then(d => {
        ...
        // Start unified poll => aggregatorTxId => depositUpdateFunction
  ...
  fetch(`${BACKEND_URL}/api/all_cryptos`)
    ...
    .then(data => {
      ...
      updateUIAfterDirectionChange();
      updateAmounts();
      // keep the 10s refresh for "You Get"
  ...
  /****************************************************
   * trackUpdateFunction => aggregatorPollManager
   ****************************************************/
