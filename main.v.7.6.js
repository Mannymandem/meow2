 ...
 *   - The dropdown is positioned absolutely within .swap-window
 *   - top:'110%' to place it just below the container
 *   - left:'50%', transform:'translateX(-50%)' for centering
 *   - width:'90%' so it doesn't overflow screen
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    // 1) Make sure the .swap-window container is position: relative
    //    so we can anchor the dropdown inside it absolutely.
    const container = document.querySelector('.swap-window');
    if(container){
    ...
    // Position the dropdown absolutely within .swap-window
    dropdown.style.position='absolute';
    dropdown.style.left='50%';
    dropdown.style.top='110%';      // place it just below the container
  ...
  // aggregatorTxId => confirm => poll => uses /api/tracking
  if(trackConfirmBtn){
