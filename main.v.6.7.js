 ...
 * 2) #tracking-modal-container => replicate deposit for tracking
 * 
 * CHANGES:
 *  - Ensures aggregator addresses are returned from /api/status/<agg_id>.
 *  - Reinstates the dropdown search function with setupSearch(...).
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  /****************************************************
   * 9) setupSearch => filter items by search text
   ****************************************************/
  function setupSearch(sInput,dropdown){
    if(!sInput) return; // if the input doesn't exist, skip
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    if(!searchContainer){
      ...
      // Depending on which dropdown we are building
  ...
  function initializeDropdowns(){
    ...
    // IMPORTANT: wire up the input event => filter
  ...
  function fetchAndUpdateTrack(aggTxId){
    ...
    // Using /api/status or /api/tracking (depending on your backend)
    fetch(`${BACKEND_URL}/api/status/${aggTxId}`)
      .then(r=>r.json())
      .then(d=>{
        hideTrackSpinner();
