document.addEventListener('DOMContentLoaded', () => {
  ...
  //----------------------------------------------------------------
  // 1) Toggle isFixed from true<->false
  //----------------------------------------------------------------
  if (rateToggleButton) {
  ...
  //----------------------------------------------------------------
  // 2) 10-min Countdown for fixed swaps
  //----------------------------------------------------------------
  ...
  //----------------------------------------------------------------
  // 10) Update "You Get" in real-time
  //----------------------------------------------------------------
  ...
  function pollTransactionStatus(aggDbIdOrString) {
    // We'll do continuous polling every 5s
  ...
  //----------------------------------------------------------------
  // 14) Switch direction (crypto->XMR or XMR->crypto)
  //----------------------------------------------------------------
  ...
  //----------------------------------------------------------------
  // 16) Initialize aggregator cryptos
  //----------------------------------------------------------------
  ...
  //----------------------------------------------------------------
  // 17) "Track" step => aggregatorTxId as string
  //----------------------------------------------------------------
  if (trackLink) {
  ...
  if (trackConfirmBtn) {
    trackConfirmBtn.addEventListener('click', () => {
      ...
      // We call /api/status/<string:aggregatorTxId>
      fetch(`${BACKEND_URL}/api/status/${aggregatorTxId}`)
        ...
        .then(data => {
          ...
          // data => { status, currency_from, currency_to, amount_from, amount_to, address_to, etc. }

          // Hide track-step, show deposit-step
          if (trackStep) trackStep.style.display = 'none';
          depositStep.style.display = 'block';

          // Show aggregatorTxId in the deposit step
          ...
          // Instead of aggregator's deposit address, show the aggregator's address_to:
          if (data.address_to) {
            depositAddressDisplay.textContent = data.address_to;
          } else {
            depositAddressDisplay.textContent = "--";
          }

          // Poll aggregator status with aggregatorTxId
