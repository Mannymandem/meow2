document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCryptoButton(buttonEl, symbol, image, network) {
    ...
    // Use coin image if available, else fallback
    const imgSrc = (image && image.trim() !== '') 
      ? image 
      : `https://static.simpleswap.io/images/currencies-logo/${symbol.toLowerCase()}.svg`;
    ...
    let displayName = coin && coin.name ? symbol.toUpperCase() + " - " + coin.name : symbol.toUpperCase();
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect) {
    ...
    cryptos.forEach(coin => {
      ...
      const imgSrc = (coin.image && coin.image.trim() !== '') 
        ? coin.image 
        : `https://static.simpleswap.io/images/currencies-logo/${coin.symbol.toLowerCase()}.svg`;
      ...
      let displayName = coin.symbol.toUpperCase();
      if (coin.name) displayName = coin.symbol.toUpperCase() + " - " + coin.name;
  ...
  exchangeButton.addEventListener('click', () => {
    ...
    .then(data => {
      ...
      // Show the exchange status container
      const exchangeStatusContainer = document.getElementById('exchange-status-container');
      if (exchangeStatusContainer) {
        exchangeStatusContainer.style.display = 'block';
      }
    ...
    const interval = setInterval(() => {
      fetch(`${BACKEND_URL}/api/status/${txId}`)
        ...
        .then(statusData => {
          ...
          statusText.classList.remove('status-waiting', 'status-confirming', 'status-sending', 'status-finished');
