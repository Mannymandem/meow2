/***************************************************************
 * main.js - FINAL with Two Modals (Addresses+Deposit, Tracking)
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
   ...
   ****************************************************/
  const BACKEND_URL = "https://meowmeow.ngrok.app"; // Already set to your server
  ...
  let trackPollInterval = null;  // for tracking modal
  ...
  /****************************************************
   * 1) MAIN TRADING WINDOW ELEMENTS
   ****************************************************/
  ...
  // “Recipient Address” in deposit step
  ...
  // "Result area" in tracking modal
  const trackResultArea          = document.getElementById('track-result-area');
  const trackModalTxDisplay      = document.getElementById('track-modal-tx-display');
  ...
  /****************************************************
   * 8) Renders currency button in main window
   ****************************************************/
  ...
  if (addressesConfirmBtn) {
    addressesConfirmBtn.onclick = () => {
      ...
      // Replace "YOUR_API_KEY" with your actual key if required by your backend
      ...
      .then(data => {
        ...
        // aggregatorTxId => from aggregator
  ...
  // Load aggregator cryptos => then load coingecko => init
  ...
  /****************************************************
   * 20) TRACKING MODAL => #tracking-modal-container
   ****************************************************/
  function resetTrackingModal() {
    ...
    if (trackResultArea) {
      trackResultArea.style.display = 'none';
    }
    if (trackModalTxDisplay) {
      trackModalTxDisplay.textContent = "--";
    }
  ...
  function hideTrackSpinner() {
    if (trackModalSpinner) {
      trackModalSpinner.style.display = 'none';
    }
  }
  function showTrackSpinner() {
    if (trackModalSpinner) {
      trackModalSpinner.style.display = 'block';
    }
  }

  // Show the tracking modal => "Track"
  ...
  // Confirm => aggregatorTxId => poll aggregator
  if (trackConfirmBtn && trackTxIdInput && trackResultArea) {
    trackConfirmBtn.addEventListener('click', () => {
      ...
      if (trackModalTxDisplay) trackModalTxDisplay.textContent = aggregatorTxId;
      startTrackPoll(aggregatorTxId);
    });
  }

  function startTrackPoll(aggregatorTxId) {
    // Clear old
    if (trackPollInterval) {
      clearInterval(trackPollInterval);
      trackPollInterval = null;
    ...
    fetchAndUpdateTrackStatus(aggregatorTxId);
    // Then every 5s
  ...
  function fetchAndUpdateTrackStatus(aggregatorTxId) {
    ...
    fetch(url)
      ...
      .catch(err => {
        console.error("Track poll error:", err);
  ...
  /****************************************************
   * 21) CLOSE BUTTON for DEPOSIT STEP
   ****************************************************/
