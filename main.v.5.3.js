document.addEventListener('DOMContentLoaded', () => {
  ...
  //----------------------------------------------------------------
  // Toggle isFixed from true<->false on each click
  //----------------------------------------------------------------
  if (rateToggleButton) {
  ...
  //----------------------------------------------------------------
  // Renders currency button in the main trading window
  //----------------------------------------------------------------
  ...
  function pollTransactionStatus(aggDbIdOrString) {
    // If you want continuous polling => setInterval logic:
  ...
  //----------------------------------------------------------------
  // 15) Show/hide currency dropdowns
  //----------------------------------------------------------------
    ...
    trackConfirmBtn.addEventListener('click', () => {
      ...
      fetch(`${BACKEND_URL}/api/status/${aggregatorTxId}`)
        ...
        .then(data => {
          ...
          // data => { status, currency_from, currency_to, amount_from, amount_to, ... }

          if (trackStep) trackStep.style.display = 'none';
          depositStep.style.display = 'block';

          // aggregatorTxId => displayed in deposit step's transaction ID field
          ...
          // depositAddressDisplay => not returned by your route, so we leave it as "--" or whatever

          // Start polling aggregator status for aggregatorTxId
