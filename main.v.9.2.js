 ...
 *
 * ADDITIONAL CHANGE:
 *   - Populate the deposit step modal (approx. symbol + exchange rate note)
 *     so it matches the trading window's selection (floating vs. fixed).
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  // OPTIONAL: If you have an element in the deposit step for the rate note
  // (e.g. <div id="modal-rate-note"></div>), you can reference it here:
  // const modalRateNoteEl       = document.getElementById('modal-rate-note');
  ...
  function renderCryptoButton(btn, symbol, image, network){
    // Remove purple background => make it transparent
    btn.innerHTML = '';
    ...
    const fallbackImg = `https://static.simpleswap.io/images/currencies-logo/${symbol.toLowerCase()}.svg`;
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect){
    ...
    cryptos.forEach(coin => {
      ...
      const fallbackImg     = `https://static.simpleswap.io/images/currencies-logo/${coin.symbol.toLowerCase()}.svg`;
      const imgSrc          = (coin.image && coin.image.trim() !== '') ? coin.image : fallbackImg;
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click', () => {
      ...
      .then(d => {
        ...
        // Optionally show the rate note if you have a #modal-rate-note element
        // const modalRateNoteEl = document.getElementById('modal-rate-note');
        // if(modalRateNoteEl){
        //   if(!isFixed){
        //     modalRateNoteEl.innerHTML = `<span style="color:#d68bfc;font-weight:bold;">Floating exchange rate</span> - <span style="color:#fff;">The amount you get may change due to market volatility.</span>`;
        //   } else {
        //     modalRateNoteEl.innerHTML = `<span style="color:#d68bfc;font-weight:bold;">Fixed exchange rate</span> - <span style="color:#fff;">The amount you get is fixed and doesn’t depend on market volatility.</span>`;
        //   }
        // }
