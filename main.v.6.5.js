 ...
 *   - FLIPPED the tracking modal addresses so it reads:
 *       track-modal-deposit-address = data.address_from
 *       track-modal-recipient-address = data.address_to
 ***************************************************************/
  ...
  function fetchAndUpdateTrack(aggTxId){
    ...
    fetch(`${BACKEND_URL}/api/status/${aggTxId}`)
      ...
      .then(d=>{
        ...
        // aggregator deposit => aggregator calls it address_from
        if(d.address_from && trackModalDepositAddrEl){
          trackModalDepositAddrEl.textContent=d.address_from;
        }
        // aggregator user => aggregator calls it address_to
