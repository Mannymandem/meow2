document.addEventListener('DOMContentLoaded', () => {
  ...
  // Potential fields inside the tracking modal
  ...
  const trackModalQrcodeEl        = document.getElementById('track-modal-qrcode');

  // The #track-link in your nav
  ...
  /****************************************************
   * 18) Switch direction => from->XMR or XMR->crypto
   ****************************************************/
  if (switchButton) {
  ...
  /****************************************************
   * 21) TRACKING MODAL => if user wants to track
   ****************************************************/
  if (trackLink) {
    trackLink.addEventListener('click', (ev) => {
      ev.preventDefault();
      // Open second tracking modal
      trackingModalContainer.style.display = 'flex';
      // Optionally reset or clear data each time
      resetTrackingModal();
    });
  }
