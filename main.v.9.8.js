 ...
 *   - The basic loading spinner has been replaced by a Lottie animation.
 *   - Polling logic unified into a single aggregatorPollManager to avoid multiple
 *     concurrent requests. Both deposit and track windows call /api/tracking/<aggTxId>
 *     once every 20 seconds if they share the same aggregatorTxId.
 *   - Dropdown search now filters only by symbol + name, ignoring the network.
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {

  /****************************************************
   * aggregatorPollManager => unify deposit + track polls
   ****************************************************/
  const aggregatorPollManager = {
    pollers: {},  // { aggTxId: { intervalId, watchers: [{ type, callback }] } }

    startPolling(aggTxId, type, callback) {
      // 1) If no poller yet, create one
      if(!this.pollers[aggTxId]) {
        this.pollers[aggTxId] = {
          intervalId: null,
          watchers: []
        };
        // Start an interval that calls /api/tracking/<aggTxId> every 20s
        this.pollers[aggTxId].intervalId = setInterval(() => {
          this._fetchAndNotify(aggTxId);
        }, 20000);

        // Immediately do one fetch so UI updates quickly
        this._fetchAndNotify(aggTxId);
      }
      // 2) Add this watcher
      this.pollers[aggTxId].watchers.push({ type, callback });
    },

    stopPolling(aggTxId, type) {
      // If no poller for that aggTxId, do nothing
      if(!this.pollers[aggTxId]) return;
      // Remove watchers of this type
      this.pollers[aggTxId].watchers = this.pollers[aggTxId].watchers.filter(w => w.type !== type);
      // If watchers array is now empty, clear interval
      if(this.pollers[aggTxId].watchers.length === 0) {
        clearInterval(this.pollers[aggTxId].intervalId);
        delete this.pollers[aggTxId];
      }
    },

    _fetchAndNotify(aggTxId) {
      // Call /api/tracking/<aggTxId> once, then pass data to watchers
      fetch(`${BACKEND_URL}/api/tracking/${aggTxId}`)
        .then(r => r.json())
        .then(d => {
          // Notify watchers
          if(!this.pollers[aggTxId]) return; // might have been cleared in the meantime
          const watchers = this.pollers[aggTxId].watchers;
          watchers.forEach(w => w.callback(d));
        })
        .catch(e => {
          console.log("aggregatorPollManager fetch error:", e);
          // We can pass an error object to watchers if needed
          if(!this.pollers[aggTxId]) return;
          const watchers = this.pollers[aggTxId].watchers;
          watchers.forEach(w => w.callback({ error: e.toString() }));
        });
    }
  };

  // Initialize Lottie animation for the modal spinner
  let lottieAnimation;
  ...
  if (spinnerContainer) {
    lottieAnimation = lottie.loadAnimation({
      ...
      loop: true,
      autoplay: true,
      path: 'https://ik.imagekit.io/mupgznku9/Animation%20-%201740216351512.json'
    });
  }
  ...
  let coingeckoMap      = {};
  // Remove tradePollInterval, trackPollInterval => replaced by aggregatorPollManager
  ...
  /****************************************************
   * 9) setupSearch => only name + symbol
   ****************************************************/
  function setupSearch(sInput, dropdown){
    ...
    sInput.addEventListener('input', () => {
      ...
      items.forEach(item => {
        // Only match name+symbol => stored in data-search
  ...
  /****************************************************
   * 10) buildDropdownItems => store data-search
   ****************************************************/
  function buildDropdownItems(dropdown, cryptos, onSelect){
    ...
    cryptos.forEach(coin => {
      ...
      // Store only name+symbol => ignoring network
      const combinedSearch = (coin.symbol + ' ' + (coin.name||'')).toLowerCase();
      itemEl.setAttribute('data-search', combinedSearch);
      ...
      // We'll still display the network pill for clarity, but we won't search by it
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click', () => {
      ...
      .then(d => {
        ...
        // Instead of pollTransactionStatus, unify with aggregatorPollManager
        aggregatorPollManager.startPolling(aggregatorTxId, "deposit", depositUpdateFunction);
      })
      .catch(e => {
  ...
  /****************************************************
   * depositUpdateFunction => used by aggregatorPollManager
   ****************************************************/
  function depositUpdateFunction(d){
    // If there's an error
    if(d.error){
      if(statusText) statusText.textContent = `Error: ${d.error}`;
      return;
    }
    // If spinnerEl is hidden or shown
    const spinnerEl = document.getElementById('modal-spinner');
    if(spinnerEl) spinnerEl.style.display = 'none';

    // d.status => waiting, confirming, exchanging, sending, finished, expired, etc.
    if(!d.status){
      if(statusText) statusText.textContent = "Unknown Status";
      return;
    }
    ...
    switch(d.status){
      ...
      case 'finished':
        ...
        // stop polling
        aggregatorPollManager.stopPolling(d.id, "deposit");
  ...
  fetch(`${BACKEND_URL}/api/all_cryptos`)
    ...
    .then(data => {
      ...
      updateUIAfterDirectionChange();
      updateAmounts();
      // We keep the 10s refresh for "You Get" only
  ...
  if(trackConfirmBtn){
    trackConfirmBtn.addEventListener('click', () => {
      ...
      // Start unified poll
      aggregatorPollManager.startPolling(aggregatorTxId, "track", trackUpdateFunction);
    });
  }
  if(trackCloseBtn){
  ...
  function resetTrackingDeposit(){
    aggregatorPollManager.stopPolling((trackModalTxIdEl?.textContent||"").trim(), "track");
  ...
  /****************************************************
   * trackUpdateFunction => used by aggregatorPollManager
   ****************************************************/
  function trackUpdateFunction(d){
    if(d.error){
    ...
    if(fromCoinData){
      const fallbackSendImg = `https://ik.imagekit.io/mupgznku9/crypto/${fromCoinData.symbol.toLowerCase()}.svg`;
      trackModalYouSendIcon.src = fallbackSendImg;
      const netKey = (fromCoinData.network || fromCoinData.symbol).toUpperCase();
    ...
    if(toCoinData){
      const fallbackGetImg = `https://ik.imagekit.io/mupgznku9/crypto/${toCoinData.symbol.toLowerCase()}.svg`;
      trackModalYouGetIcon.src = fallbackGetImg;
      const netKey2 = (toCoinData.network || toCoinData.symbol).toUpperCase();
    ...
    switch(d.status){
      ...
      case 'finished':
        ...
        aggregatorPollManager.stopPolling(d.id, "track");
        break;
      case 'expired':
  ...
  if(tradeCloseBtn && tradeModalContainer){
    tradeCloseBtn.addEventListener('click', () => {
      tradeModalContainer.style.display = 'none';
      // Also stop polling for deposit if user closes
      if(transactionIdEl && transactionIdEl.textContent.trim()){
        aggregatorPollManager.stopPolling(transactionIdEl.textContent.trim(), "deposit");
      }
    });
  }
