 ...
 *
 * FURTHER CHANGE:
 *   - Removed aggregator-provided images entirely, using only
 *     ImageKit fallback for all crypto icons.
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mapping for URI schemes based on currency symbol
  const uriSchemes = {
    "BTC": "bitcoin:",
    "ETH": "ethereum:",
    "XMR": "monero:"
  ...
  // Make the text fields invisible / no white box
  if(fromAmountInput){
  ...
  if (rateToggleButton) {
    ...
    // Tippy.js tooltip for fixed/float explanation
   ...
   ****************************************************/
  function renderCryptoButton(btn,symbol/* aggregator ignores image */,_,network){
    // We ignore aggregator's coin.image, always using fallback from imagekit
    btn.innerHTML='';
    btn.style.display='inline-flex';
    btn.style.alignItems='center';
    btn.style.padding='10px';
    btn.style.background='transparent'; // updated
  ...
  function setupSearch(sInput, dropdown){
    ...
    // Make the search border + text #d68bfc, with border-radius
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    // Identify which .swap-input container to anchor to:
    let parentInputDiv=null;
    if(dropdown===fromCurrencyDropdown){
      // "send" container is .swap-input.send
      parentInputDiv = document.querySelector('.swap-input.send');
    } else {
      // "receive" container is .swap-input.receive
      parentInputDiv = document.querySelector('.swap-input.receive');
    }

    if(parentInputDiv){
      // Make sure parent has position: relative so we can anchor absolutely inside it
    ...
    // Now position the dropdown absolutely inside the parent .swap-input
    dropdown.style.position='absolute';
    dropdown.style.left='50%';
    // place it just below the .swap-input
    ...
    dropdown.style.display='none'; // hide by default, toggled later
    ...
    cryptos.forEach(coin=>{
      ...
      // “Card” => transparent, smaller, single row
      ...
      // We ignore aggregator coin.image => always fallback
  ...
  function updateUIAfterDirectionChange(){
    if(direction==="crypto_to_xmr"){
      ...
      // aggregator coin.image is ignored => no param usage
      renderCryptoButton(fromCurrencyButton, fromCoin.symbol, null, fromCoin.network);

      let xmrCoin=aggregatorCryptos.find(c=>c.symbol==="XMR");
      if(!xmrCoin) xmrCoin={symbol:"XMR", image:"", network:"xmr"};
      renderCryptoButton(toCurrencyButton, xmrCoin.symbol, null, xmrCoin.network);

      // disable pointer on the "Receive" button if direction is crypto->xmr
      toCurrencyButton.style.pointerEvents='none';
      fromCurrencyButton.style.pointerEvents='auto';
    } else {
      let xmrCoin=aggregatorCryptos.find(c=>c.symbol==="XMR");
      if(!xmrCoin) xmrCoin={symbol:"XMR", image:"", network:"xmr"};
      renderCryptoButton(fromCurrencyButton, xmrCoin.symbol, null, xmrCoin.network);
      ...
      renderCryptoButton(toCurrencyButton, toCoin.symbol, null, toCoin.network);
  ...
  function updateWarnings(){
    ...
    let fromCur,toCur;
    if(direction==="crypto_to_xmr"){
      fromCur=selectedFromCurrency; toCur="XMR";
    } else {
      fromCur="XMR"; toCur=selectedToCurrency;
    }
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click',()=>{
      // Enforce the mandatory checkbox before continuing:
      ...
      .then(d=>{
        ...
        // Always use imagekit fallback for "You Send"
        if(fromCoinData){
        ...
        // QR: Generate with URI scheme, black/white color, and high error correction.
        if(qrcodeContainer && d.deposit_address){
          qrcodeContainer.innerHTML='';
          let depositCurrency = (direction === "crypto_to_xmr") 
                                 ? selectedFromCurrency 
                                 : "XMR";
          let scheme = uriSchemes[depositCurrency.toUpperCase()] 
                       || depositCurrency.toLowerCase() + ":";
        ...
        // poll aggregator (this is the deposit step)
   ...
   ****************************************************/
  // We keep the direction-based logic that only
  // opens the from dropdown if direction is crypto->xmr,
  // and only opens the to dropdown if direction is xmr->crypto.
  ...
  document.addEventListener('click',(ev)=>{
    if(fromCurrencyDropdown 
       && !fromCurrencyDropdown.contains(ev.target)
       && !fromCurrencyButton.contains(ev.target)){
      fromCurrencyDropdown.style.display='none';
    }
    if(toCurrencyDropdown 
       && !toCurrencyDropdown.contains(ev.target)
       && !toCurrencyButton.contains(ev.target)){
  ...
  // aggregatorTxId => confirm => poll => /api/tracking
  if(trackConfirmBtn){
    ...
    fetch(`${BACKEND_URL}/api/tracking/${aggTxId}`)
      ...
      .then(d=>{
        ...
        if(d.amount_to && trackModalYouGetAmount){
          let displayAmt = d.amount_to.toString();
          // If the API returns type "floating", prepend "~" to the amount if not already present
          if(d.type === "floating" && !displayAmt.startsWith("~")) {
            displayAmt = "~" + displayAmt;
          }
          trackModalYouGetAmount.textContent = displayAmt;
        }
