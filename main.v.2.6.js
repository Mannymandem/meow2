document.addEventListener('DOMContentLoaded', () => {
  ...
  // Renders a custom button with symbol & network
  ...
  // Builds the dropdown with cryptos
  ...
  // Re-render UI after direction change
  ...
  // Basic fallback warnings from aggregator currency metadata
  ...
  // Exchange button -> 2-step modal flow
  exchangeButton.addEventListener('click', () => {
    ...
    addressesConfirmBtn.onclick = () => {
      ...
      .then(data => {
        ...
        // Insert aggregator_tx_id raw => no "Transaction ID: "
        if (transactionIdEl) {
          transactionIdEl.textContent = data.aggregator_tx_id;
        }
        // Insert deposit_address raw => no "Deposit Address: "
        ...
        startDepositCountdown(); // Start 10-min countdown
      })
      .catch(err => {
