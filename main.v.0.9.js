document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCryptoButton(buttonEl, symbol, image, network) {
    ...
    // Use coin image if available, else fallback to coingecko, else static SimpleSwap
    ...
    let displayName = coin && coin.name ? uppercaseSymbol + " - " + coin.name : uppercaseSymbol;
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect) {
    ...
    cryptos.forEach(coin => {
      ...
      let displayName = uppercaseSymbol;
      if (coin.name) displayName = uppercaseSymbol + " - " + coin.name;
  ...
  fetch(`${BACKEND_URL}/api/all_cryptos`)
    ...
    .then(cryptos => {
      ...
      // Now fetch CoinGecko data for fallback logos
