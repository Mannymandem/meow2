document.addEventListener('DOMContentLoaded', () => {
  ...
  // Instead of adding brackets in the button symbol, we'll just show the main ticker and a separate network pill
  // For example: USDTBEP20 on BSC -> Ticker: USDT, Network Pill: BSC
  function formatTickerAndNetwork(symbol, network) {
    let s = symbol.toUpperCase();
    let net = network ? network.toUpperCase() : '';

    // If the symbol ends with BEP20 and network is BSC, remove BEP20 from symbol
  ...
  function renderCryptoButton(buttonEl, symbol, image, network) {
    ...
    // Add a network pill consistently sized
    if (net) {
      const netKey = net;
      ...
      networkPill.style.minWidth = '40px';
  ...
  function setupSearch(searchInput, dropdown) {
    // Make the search bar sticky within the dropdown
    // Assuming searchInput is at the top of the dropdown
    ...
    if (!itemsContainer) {
      ...
      // Move all dropdown-item elements into this container
      // We'll do this after building items
    }
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect) {
    ...
    // Ensure the search input stays at the top: 
    // We assume searchInput is a child of dropdown
    // Wrap search input in a sticky container
    ...
    if (!searchContainer) {
      ...
      if (dropdown.contains(fromSearchInput) || dropdown.contains(toSearchInput)) {
        // Determine which search input belongs here
    ...
    if (!itemsContainer) {
      ...
      // 300px total height - search bar height (approx) to keep search visible
