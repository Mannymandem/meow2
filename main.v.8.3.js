...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateAmounts(){
    ...
    fetch(url)
      ...
      .then(d=>{
        ...
        if(toAmountInput){
          // Here is the minimal change: add "~" if isFixed is false
  ...
  // Create a wrapper div for margin/padding
  const qrWrapper = document.createElement('div');
  // Add some padding and a white background for extra margin/contrast
  qrWrapper.style.padding = '10px';
  qrWrapper.style.background = '#fff';
  qrWrapper.style.display = 'inline-block';

  // Append this wrapper to the qrcodeContainer
  qrcodeContainer.appendChild(qrWrapper);

  // Figure out the deposit currency and scheme
  ...
  // Finally generate the QR code inside the wrapper
