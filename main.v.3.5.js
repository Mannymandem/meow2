document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== NEW: We'll store isFixed in a variable that toggles
  let isFixed = false; // default to "Float" unless user chooses float
  ...
  // ============ Toggle Button for Rate (Float vs Fixed) ============
  const toggleFloatEl = document.getElementById('toggle-float'); // "Float" button
  const toggleFixedEl = document.getElementById('toggle-fixed'); // "Fixed" button
  ...
  // ------------------------------------------------
  // 1) Rate Toggle: float vs fixed
  // ------------------------------------------------
  // By default isFixed = true. If user clicks "Float", we set isFixed=false, etc.
  toggleFloatEl.addEventListener('click', () => {
    isFixed = false;
    // update UI highlight
    toggleFloatEl.classList.add('rate-toggle-active');   // a custom class you might add
    toggleFixedEl.classList.remove('rate-toggle-active');
    updateAmounts(); // re-fetch estimate with float
  });

  toggleFixedEl.addEventListener('click', () => {
    isFixed = true;
    // update UI highlight
    toggleFixedEl.classList.add('rate-toggle-active');
    toggleFloatEl.classList.remove('rate-toggle-active');
    updateAmounts(); // re-fetch estimate with fixed
  });
  ...
  addressesConfirmBtn.onclick = () => {
    ...
    const payload = {
      ...
      is_fixed: isFixed // pass isFixed => true/false
    };
