/***************************************************************
 * main.js - TWO MODALS: Exchange & Tracking (EXACT DEPOSIT REPLICA)
 * 
 * CHANGES to fix wallet addresses in tracking:
 *  - Fill #track-modal-deposit-address = data.address_to
 *  - Fill #track-modal-recipient-address = data.address_from
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  /****************************************************
   * 3) TRACKING MODAL => EXACT DEPOSIT REPLICA
   ****************************************************/
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click',()=>{
      ...
      // if your backend needs an api_key param
  ...
  // aggregatorTxId step => CONFIRM => poll
  if(trackConfirmBtn){
    trackConfirmBtn.addEventListener('click',()=>{
      ...
      // hide aggregatorTxId step, show deposit replicate
  ...
  function fetchAndUpdateTrack(aggTxId){
    ...
    fetch(`${BACKEND_URL}/api/status/${aggTxId}`)
      ...
      .then(d=>{
        ...
        // aggregator deposit address => address_to
        if(d.address_to && trackModalDepositAddrEl){
          trackModalDepositAddrEl.textContent=d.address_to;
        }
        // aggregator "recipient" => address_from
        if(d.address_from && trackModalRecipientAddrEl){
          trackModalRecipientAddrEl.textContent=d.address_from;
        }
