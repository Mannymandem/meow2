...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let isFixed            = true;  // single toggle: "locked" = fixed rate

  // Instead of re-declaring countdownInterval multiple times,
  // let's declare it once here:
  ...
  function renderCryptoButton(buttonEl, symbol, image, network) {
    // same as before, make sure not declared twice
    // ...
    buttonEl.innerHTML = '';
    // ...
    // do the appended elements + network pill
    // ...
  }
    ...
    const interval = setInterval(() => {
      fetch(`${BACKEND_URL}/api/status/${txId}`)
        ...
        .then(statusData => {
          ...
          // if no longer waiting => stop countdown
  ...
  // ================================
  // 12) Start everything on page load
  // ================================
  // Setup searches
  ...
  // Actual fetch for aggregatorCryptos
  fetch(`${BACKEND_URL}/api/all_cryptos`)
    ...
    .catch(err => {
      ...
      // aggregatorCryptos is empty => fallback logic if needed
    });
});
