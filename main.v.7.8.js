...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Instead of a single element by id, we now select all track nav links by class
  const trackNavLinks             = document.querySelectorAll('.track-nav');
    ...
    fetch(url)
      ...
      .then(d => {
        ...
        if(toAmountInput){
          ...
          // If floating rate (isFixed false), prepend an approximate symbol
          if(!isFixed){
   ...
   ****************************************************/
  // Instead of a single element, attach event listeners to all track nav links found on the page.
  if(trackingModalContainer && trackNavLinks.length > 0){
