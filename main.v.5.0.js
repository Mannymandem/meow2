document.addEventListener('DOMContentLoaded', () => {
  ...
  // "You Send" / "You Receive" in the modal
  ...
  const toWarningEl   = document.getElementById('network-warning-to'); // If used

  // Remove or ignore any references to "snake" steps
  // (We no longer have snakeStatusText, snakeSpinner in the new HTML)
  ...
  //---------------------------------------------------------------------------
  // 1) Toggle isFixed from true<->false on each click of the rate toggle button
  //---------------------------------------------------------------------------
  if (rateToggleButton) {
    rateToggleButton.addEventListener('click', () => {
      ...
      // Re-fetch the updated amounts whenever the swap mode changes
      updateAmounts();
    });
  }

  // Ensure paragraph says "Rate = fixed" initially
  if (rateStatusEl) {
    rateStatusEl.textContent = isFixed ? "Rate = fixed" : "Rate = float";
  }

  //------------------------------------------------------
  // 2) 10-min Countdown - only show if isFixed = true
  //------------------------------------------------------
  function startDepositCountdown() {
    // If the swap is not fixed, hide countdown timer and return
  ...
  //------------------------------------------------------
  // 5) Renders currency button in the main trading window
  //------------------------------------------------------
  ...
  //------------------------------------------------------
  // 9) aggregator warnings (network mismatch, etc.)
  //------------------------------------------------------
  ...
  //------------------------------------------------------
  // 10) Update "You Get" amount in real-time
  //------------------------------------------------------
  ...
  //------------------------------------------------------
  // 11) "Exchange" => show addresses modal
  //------------------------------------------------------
  ...
  addressesConfirmBtn.onclick = () => {
    ...
    .then(data => {
      ...
      // Update "You Send" details in the modal
      ...
      modalYouSendAmount.textContent = fromAmount.toString(); // just numeric

      // Update "You Receive" details in the modal
      ...
      if (toCoinData && data.to_amount) {
        ...
        modalYouGetAmount.textContent = data.to_amount.toString(); // just numeric
      }
  ...
  //------------------------------------------------------
  // 13) Switch direction (crypto->XMR or XMR->crypto)
  //------------------------------------------------------
