 ...
 *  1) Made "from" & "to" text fields transparent (no white box).
 *  2) Changed dropdown background to #2a013f, each card is transparent, single row.
 *  3) Made search bar mostly invisible except a faint outline (border).
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  // === Make text fields invisible / no white box
  if(fromAmountInput){
  ...
  function setupSearch(sInput, dropdown){
    if(!sInput) return;
    // Make the search bar basically invisible, faint outline
    sInput.style.background = 'transparent';
    sInput.style.border = '1px solid #666';
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    if(!searchContainer){
      ...
      // Instead of the old purple, let's make it transparent
    ...
    cryptos.forEach(coin=>{
      ...
      // Make the “card” transparent & smaller
      itemEl.style.display='flex';
      itemEl.style.alignItems='center';
      itemEl.style.justifyContent='flex-start';
      itemEl.style.cursor='pointer';
      itemEl.style.padding='5px';  // reduced padding
      itemEl.style.background='transparent'; // transparent card
      itemEl.style.borderRadius='5px';
      itemEl.style.marginBottom='5px'; // reduced spacing
      ...
      // Make everything in a single row
