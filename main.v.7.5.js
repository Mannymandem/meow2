 ...
 *   - Dropdown is now centered + same width as the container
 *   - Removed #9002c0 background from crypto button => transparent
 *   - Search bar border + text are now #d68bfc, with rounded corners
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCryptoButton(btn,symbol,image,network){
    ...
    btn.style.background='transparent'; // NEW
  ...
  function setupSearch(sInput, dropdown){
    ...
    sInput.style.border = '1px solid #d68bfc'; // NEW
    sInput.style.borderRadius = '8px';         // NEW
    sInput.style.color = '#d68bfc';            // NEW
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    // We want the dropdown to be centered + same width as “trading window.”
    // So let's find the parent container, e.g. the "swap-window" or something.
    // We'll approximate by setting .style.width='90%', left='50%', transform='translateX(-50%)'.
    dropdown.style.position='absolute';
    dropdown.style.left='50%';
    dropdown.style.transform='translateX(-50%)';
    dropdown.style.width='90%'; // NEW
