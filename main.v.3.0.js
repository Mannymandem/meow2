document.addEventListener('DOMContentLoaded', () => {
  ...
  // Input fields in the main trading window
  ...
  // Modal elements: container + steps
  ...
  // ---------------------------------------------------------
  // 1) 10-minute Countdown Logic: Start / Stop
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // 2) Parse aggregator errors / warnings
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // 4) Renders the currency button (Symbol, Logo, Network)
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // 7) Update the UI after direction change (crypto <-> xmr)
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // 8) aggregator warnings: fetch network info & display
  // ---------------------------------------------------------
  ...
  // merges aggregator warnings into the deposit modal
  ...
  // ---------------------------------------------------------
  // 9) Update "You Get" when user enters fromAmount
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // 10) “Exchange” button => show addresses step
  // ---------------------------------------------------------
  ...
  // On "Confirm" addresses => create exchange
  addressesConfirmBtn.onclick = () => {
    ...
    .then(data => {
      ...
      // poll aggregator & start countdown
  ...
  // ---------------------------------------------------------
  // 11) Poll aggregator status => includes snake logic
  // ---------------------------------------------------------
    ...
    const interval = setInterval(() => {
      fetch(`${BACKEND_URL}/api/status/${txId}`)
        ...
        .then(statusData => {
          ...
          // snake spinner
          if (snakeSpinner) {
            snakeSpinner.style.display = 'none';
          }
          if (snakeStatusText) {
            snakeStatusText.className = '';
          }

          // Show spinner if waiting-ish
          ...
          // Mirror the same status in the Snake step
          if (snakeStatusText) {
            // remove old class
            snakeStatusText.className = '';
            switch(statusData.status) {
  ...
  // ---------------------------------------------------------
  // 13) Fetch aggregator cryptos & fallback logos
  // ---------------------------------------------------------
  fetch(`${BACKEND_URL}/api/all_cryptos`)
    ...
    .then(data => {
      ...
      // auto-refresh estimate every 10s if user remains on page
      setInterval(() => {
        updateAmounts();
      }, 10000);
    })
