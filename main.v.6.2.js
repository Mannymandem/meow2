/***************************************************************
 * main.js - with Two Modals: Exchange & Tracking (EXACT REPLICA)
 * 
 * CHANGES:
 *  1) In the tracking poll (fetchAndUpdateTrack),
 *     we fill icons & pills for “You Send / You Receive,”
 *     plus “Recipient Address.”
 *  2) We add #track-close-btn to let user close
 *     from the deposit replicate step.
 *  3) Everything else is the same aggregator logic.
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
   ...
   * 0) GLOBAL CONFIG
   ****************************************************/
  const BACKEND_URL = "https://meowmeow.ngrok.app";
  ...
  // let's also add a "Recipient Address" for tracking
   ...
   ****************************************************/
  function renderCryptoButton(btnEl, symbol, image, network) {
    btnEl.innerHTML = '';
    btnEl.style.display='inline-flex'; 
    btnEl.style.alignItems='center';
    btnEl.style.padding='10px'; 
    btnEl.style.background='#9002c0'; 
    btnEl.style.border='none'; 
    btnEl.style.color='#fff'; 
    btnEl.style.margin='0 8px 0 0';
  ...
  function buildDropdownItems(dropdown, cryptos, onSelect){
    const ex=dropdown.querySelectorAll('.dropdown-item');
  ...
  async function fetchWarnings(symbol){
    try{
      ...
      const net = d.network||"";
      if(!net)return"";
      const nk=net.toUpperCase();
      const bg=networkColors[nk]||"#444";
  ...
  function updateAmounts(){
    ...
    fetch(url)
      ...
      .then(d=>{
        if(d.error){
          if(toAmountInput) toAmountInput.value=parseErrorDescription(d.error);
          return;
        }
        if(toAmountInput){
          toAmountInput.value=d.to_amount.toFixed(6);
        }
      })
      .catch(e=>{
        console.log("updateAmounts fetch err:",e);
  ...
  if(addressesConfirmBtn){
    addressesConfirmBtn.addEventListener('click',()=>{
      ...
      .then(d=>{
        ...
        if(transactionIdEl) transactionIdEl.textContent=aggregatorTxId;
        if(depositAddressDisplay) depositAddressDisplay.textContent=d.deposit_address||"--";
        ...
        // fill deposit recipient
        if(modalRecipientAddrEl) modalRecipientAddrEl.textContent=addressInput||"--";
  ...
  /****************************************************
   * 16) aggregator poll => deposit step
   ****************************************************/
  ...
  /****************************************************
   * 19) LOAD aggregator cryptos => fill
   ****************************************************/
  ...
  // track CONFIRM => aggregatorTxId => poll
  if(trackConfirmBtn){
    trackConfirmBtn.addEventListener('click',()=>{
      ...
      // poll aggregator => fill
      pollTrackingStatus(aggregatorTxId);
    });
  }

  // track CLOSE => from the deposit replicate step
  if(trackCloseBtn){
  ...
  // fill the track deposit replicate
  function fetchAndUpdateTrack(aggTxId){
    ...
    fetch(`${BACKEND_URL}/api/status/${aggTxId}`)
      ...
      .then(d=>{
        ...
        // try to find aggregator crypto => fill icon/pill
        ...
        // aggregator deposit address?
        if(d.address_to && trackModalDepositAddrEl){
          trackModalDepositAddrEl.textContent=d.address_to;
        }
        // aggregator recipient address?
        if(d.recipient_address && trackModalRecipientAddrEl){
          // or if(d.address_recipient)
          trackModalRecipientAddrEl.textContent=d.recipient_address;
        }
