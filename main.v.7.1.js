 ...
 * main.js - Final 
 * 
 * 1) #exchange-modal-container => addresses + deposit steps
 * 2) #tracking-modal-container => exact deposit replicate
 * 
 * CHANGES:
 *   - "Deposit step" polling uses /api/status/<agg_id>
 *   - "Tracking modal" calls /api/tracking/<agg_id>", so we can get
 *     address_from/address_to for display.
 *   - Old dropdown search code reintroduced
 *   - Tippy.js tooltip added to #rate-toggle-button
 *   - parseErrorDescription(...) now checks for Max first if both Min & Max exist
 ***************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  function parseErrorDescription(errMsg){
    // Attempt to parse aggregator's JSON from the substring if it includes { ...
    ...
    if(jsStart>-1){
      ...
      try{
        ...
        if(parsed.description){
          ...
          // If it includes "Amount does not fall within the range."
          // we check for Min: and Max:. If both exist, show max first.
          if(desc.includes("Amount does not fall within the range.")){
            const minMatch = desc.match(/Min:\s*([\d\.]+)/);
            const maxMatch = desc.match(/Max:\s*([\d\.]+)/);

            // If there's a max line, we prefer that:
            if(maxMatch && maxMatch[1]) {
              return `Max Amount: ${maxMatch[1]}`;
            }
            // else if there's a min line, show that:
            if(minMatch && minMatch[1]) {
              return `Min Amount: ${minMatch[1]}`;
            }
          }
          return desc;
        }
  ...
  function buildDropdownItems(dropdown,cryptos,onSelect){
    ...
    if(!searchContainer){
      ...
      sInput.style.borderRadius='5px';
      sInput.style.border='none';
      sInput.style.fontSize='14px';
      sInput.style.outline='none';
      sInput.style.color='#000';
  ...
  // fetch all cryptos => then coingecko => init
  ...
  // aggregatorTxId => confirm => poll => uses *new* /api/tracking
  if(trackConfirmBtn){
  ...
  // aggregator poll => track (calls /api/tracking/<aggId>)
  ...
  function fetchAndUpdateTrack(aggTxId){
    ...
    // Instead of /api/status, we call the new /api/tracking route:
    fetch(`${BACKEND_URL}/api/tracking/${aggTxId}`)
      ...
      .then(d=>{
        ...
        // aggregator deposit address => address_from
