    ...
    const interval = setInterval(() => {
      fetch(`${BACKEND_URL}/api/status/${txId}`)
        ...
        .then(statusData => {
          ...
          // Based on the snippet provided by SimpleSwap, we have statuses like:
          // waiting, confirming, sending, finished
          // 'exchanging' might not actually appear in this aggregator response,
          // but we'll keep handling it gracefully if it does appear.
          switch(statusData.status) {
            case 'waiting':
              ...
              break;
            case 'confirming':
              // If aggregator does not provide confirmations, we simply won't show them
              modalStatusText.classList.add('status-confirming');
              modalStatusText.textContent = "Confirming";
              // Only show confirmations if provided
